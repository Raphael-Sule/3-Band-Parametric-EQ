...
: AudioProcessor(BusesProperties() #if ! JucePlugin_IsMidiEffect
...
{
#if JucePlugin_WantsMidiInput return true;
#else
return false; #endif
}
...
{
#if JucePlugin_IsMidiEffect return true;
#else
return false; #endif
}
...
// this prepares each chain to be prepared for processing juce::dsp::ProcessSpec shem;
shem.maximumBlockSize = samplesPerBlock; shem.numChannels = 1;
shem.sampleRate = sampleRate;

leftChain.prepare(shem); rightChain.prepare(shem);
...
, getchainSettings.peakFreq, getchainSettings.peakQuality, juce::Decibels::decibelsToGain(getchainSettings.peakGain));
...
auto lowCutCoefficients = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButt erworthMethod(getchainSettings.lowCutFreq, sampleRate, (getchainSettings.lowCutSlope + 1) * 2);

auto& RightLowCut = rightChain.get<LowCut>(); auto& leftLowCut = leftChain.get<LowCut>();

RightLowCut.setBypassed<0>(true); RightLowCut.setBypassed<1>(true); RightLowCut.setBypassed<2>(true); RightLowCut.setBypassed<3>(true);

leftLowCut.setBypassed<0>(true); leftLowCut.setBypassed<1>(true); leftLowCut.setBypassed<2>(true); leftLowCut.setBypassed<3>(true);
...
*lowCutCoefficients[3];
leftLowCut.setBypassed<3>(false); [[fallthrough]]; // fallthrough is explicit
}
...
*lowCutCoefficients[2];
leftLowCut.setBypassed<2>(false); [[fallthrough]]; // fallthrough is explicit
}
case Slope2:
{
...
*lowCutCoefficients[1];
leftLowCut.setBypassed<1>(false); [[fallthrough]]; // fallthrough is explicit
}
case Slope1:
{
...
*lowCutCoefficients[0];
leftLowCut.setBypassed<0>(false); [[fallthrough]]; // fallthrough is explicit
}
}

auto highCutCoefficients = juce::dsp::FilterDesign<float>::designIIRLowpassHighOrderButte rworthMethod(getchainSettings.highCutFreq, sampleRate, (getchainSettings.highCutSlope + 1) * 2);

auto& RightHighCut = rightChain.get<HighCut>(); auto& LeftHighCut = leftChain.get<HighCut>();

RightHighCut.setBypassed<0>(true); RightHighCut.setBypassed<1>(true); RightHighCut.setBypassed<2>(true);
 
RightHighCut.setBypassed<3>(true);

LeftHighCut.setBypassed<0>(true); LeftHighCut.setBypassed<1>(true); LeftHighCut.setBypassed<2>(true); RightHighCut.setBypassed<3>(true);
...
#ifndef JucePlugin_PreferredChannelConfigurations bool EQAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
#if JucePlugin_IsMidiEffect juce::ignoreUnused(layouts); return true;
#else
// This is the place where you check if the layout is supported.
// In this template code we only support mono or stereo.
// Some plugin hosts, such as certain GarageBand versions, will only
// load plugins that support stereo bus layouts. if (layouts.getMainOutputChannelSet() !=
...
auto getchainSettings = getchainsettings(apvts); auto peakCoefficients =
juce::dsp::IIR::Coefficients<float>::makePeakFilter(getSampleR ate(), getchainSettings.peakFreq, getchainSettings.peakQuality, juce::Decibels::decibelsToGain(getchainSettings.peakGain));
...
*leftChain.get<Peak>().coefficients = *peakCoefficients;
//as these are references, they point to the addresses of variables.
*rightChain.get<Peak>().coefficients =
*peakCoefficients;// to get the data in these variables, we must dereference them.

auto lowCutCoefficients = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButt erworthMethod(getchainSettings.lowCutFreq, getSampleRate(), (getchainSettings.lowCutSlope + 1) * 2);
...
leftLowCut.setBypassed<0>(true); leftLowCut.setBypassed<1>(true); leftLowCut.setBypassed<2>(true); leftLowCut.setBypassed<3>(true); switch (getchainSettings.lowCutSlope)
{
case Slope4:
{
*RightLowCut.get<3>().coefficients =
...
auto highCutCoefficients = juce::dsp::FilterDesign<float>::designIIRLowpassHighOrderButte rworthMethod(getchainSettings.highCutFreq, getSampleRate(), (getchainSettings.highCutSlope + 1) * 2);

auto& RightHighCut = rightChain.get<HighCut>(); auto& LeftHighCut = leftChain.get<HighCut>();

RightHighCut.setBypassed<0>(true); RightHighCut.setBypassed<1>(true); RightHighCut.setBypassed<2>(true); RightHighCut.setBypassed<3>(true);
...
auto leftBlock = block.getSingleChannelBlock(0); auto rightBlock = block.getSingleChannelBlock(1);
...
leftChain.process(leftContextReplacing); rightChain.process(rightContextReplacing);
...
Freq", "LowCut Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 20.f));

//HighCut Frequency


layout.add(std::make_unique<juce::AudioParameterFloat>("highcu tFreq", "HighCut Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 20000.f));

//Parametric Peak


layout.add(std::make_unique<juce::AudioParameterFloat>("PeakFr eq", "Peak Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 750.f));

//Peak Gain


layout.add(std::make_unique<juce::AudioParameterFloat>("peakGa in", "Peak Gain", juce::NormalisableRange<float>(-24.f, 24.f, 0.5f, 1.f), 0.0f));

//Peak Quality (How wide / narrow the peak is)


layout.add(std::make_unique<juce::AudioParameterFloat>("peakQu ality", "Peak Quality", juce::NormalisableRange<float>(0.1f, 10.f, 0.05f, 1.f), 1.f));
// stringArray dropdown table juce::StringArray stringArray;
...
layout.add(std::make_unique<juce::AudioParameterChoice>("lowcu tSlope", "LowCut Slope", stringArray, 0));
 

layout.add(std::make_unique<juce::AudioParameterChoice>("highc utSlope", "HighCut Slope", stringArray, 0));

return layout;
}
//============================================================
==================
// This creates new instances of the plugin.. juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
